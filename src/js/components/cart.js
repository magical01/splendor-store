...
const printQuantity = () => {
  ...
  if (length > 0) {
    ...
    document
      .querySelector(".cart-content__main")
      .classList.remove("cart-content__main--hide");
    document
      .querySelector(".cart-empty")
      .classList.add("cart-content__empty--hide");
  } else {
    basket.forEach((elem) => {
    ...
    document
      .querySelector(".cart-content__main")
      .classList.add("cart-content__main--hide");
    document
      .querySelector(".cart-empty")
      .classList.remove("cart-content__empty--hide");
  }
};
...
//   let currentPrice =
//     productParent?.querySelector(".card-info__price").textContent;
...
const initialState = () => {
  ...
  //   cartProdictList.innerHTML = localStorage.getItem("products");
  //   printQuantity();
  //   countSumm();
  //   printFullPrice();
  // }
...
addCart?.addEventListener("click", (e) => {
  ...
  let img = parent
    ?.querySelector(".card-slider__thumb picture img")
    .getAttribute("src");
  img = img.split(".");
  img.pop();
  img = img.join(".");
  ...
  if (selectorColor == null) {
    ...
    document
      ?.querySelector(".card-color__close")
      ?.addEventListener("click", (e) => {
        if (e.target.closest(".card-color__close")) {
          document
            ?.querySelector(".card-color")
            .classList.remove("card-color--active");
        }
      });
    return;
  }
  ...
  if (size == undefined) {
    ...
    document
      ?.querySelector(".card-size__close")
      ?.addEventListener("click", (e) => {
        if (e.target.closest(".card-size__close")) {
          document
            ?.querySelector(".card-size")
            .classList.remove("card-size--active");
        }
      });
    return;
  }
  ...
  const selectedColorCode =
    datasetColorOptions[colorNameOptions.indexOf(selectedColorName)] ||
    "#272727";
